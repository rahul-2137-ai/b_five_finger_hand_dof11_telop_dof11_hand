            ...
            session.upsert(
                [
                    ImageBackground(
                        ...
                        # Plane sized to match the pinhole camera's vertical FOV
                        # (fovy=90 -> 2*atan((height/2)/distance)=90 deg at
                        # height=2, distance=1) so the world appears at correct
                        # scale/depth, not zoomed. (For an equirect/fisheye source
                        # you'd instead need a sphere, which Vuer's flat plane
                        # cannot do.)
                        height=2,
                        distanceToCamera=1,
