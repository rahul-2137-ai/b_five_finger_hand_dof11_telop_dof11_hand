...
from teleop.image_client import ImageClient
from teleop.stereo_client import StereoImageClient
...
if __name__ == '__main__':
    ...
    parser.add_argument('--stereo-server-ip', type=str, default=None, help='IP address of the stereo camera server for VR immersive view. If None, uses --img-server-ip.')
    parser.add_argument('--stereo-port', type=int, default=5577, help='ZMQ port of the stereo camera server (VR immersive view)')
    ...
    try:
        ...
        # image client (ego_view): used ONLY for dataset recording. Do not change.
        img_client = ImageClient(host=args.img_server_ip, request_bgr=True)
        camera_config = img_client.get_cam_config()
        logger_mp.debug(f"Camera config: {camera_config}")

        # stereo client: high-res binocular pair streamed to the VR headset for an
        # immersive stereoscopic view. Display only — never recorded.
        stereo_client = StereoImageClient(host=args.stereo_server_ip or args.img_server_ip,
                                          port=args.stereo_port, request_bgr=True)
        # whether to push a locally-fetched image to the XR device (off in pass-through).
        xr_need_local_img = args.display_mode != 'pass-through'

        # televuer_wrapper: obtain hand pose data from the XR device and transmit the
        # robot's stereo camera image to the XR device. The display is driven by the
        # stereo client (binocular), so configure the wrapper from the stereo feed and
        # render locally over zmq (render_to_xr is ignored when webrtc is enabled).
        tv_wrapper = TeleVuerWrapper(use_hand_tracking=args.input_mode == "hand",
                                     binocular=stereo_client.binocular,
                                     img_shape=stereo_client.image_shape,
        ...
        while not START and not STOP: # wait for start or stop signal.
            ...
            if xr_need_local_img:
                stereo_frame = stereo_client.get_stereo_frame()
                if stereo_frame is not None:
                    tv_wrapper.render_to_xr(stereo_frame)
        ...
        while not STOP:
            ...
            # send stereo image to the VR headset (immersive view); display only.
            if xr_need_local_img:
                stereo_frame = stereo_client.get_stereo_frame()
                if stereo_frame is not None:
                    tv_wrapper.render_to_xr(stereo_frame)
            # get ego_view images for dataset recording only (unchanged img_client).
        ...
        try:
            if stereo_client is not None:
                stereo_client.close()
        except Exception as e:
            logger_mp.error(f"Failed to close stereo client: {e}")
