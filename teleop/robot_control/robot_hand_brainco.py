...
class Brainco_Controller_ctrl:
    def __init__(self, left_gripper_trigger_in, left_gripper_squeeze_in, right_gripper_trigger_in, right_gripper_squeeze_in, 
        ...
        # Finger position-loop gains sent to the BrainCo firmware.
        # kp = stiffness (grip firmness): higher -> holds the commanded close
        #      angle harder against an object, i.e. stronger grasp.
        # kd = damping: higher -> smoother / less finger oscillation.
        # Tune these for a reliable grasp; raise kp if the hand "gives" when it
        # contacts an object, raise kd if the fingers buzz/oscillate.
        self.kp = 10.0
        self.kd = 0.1
    ...
    def control_process(self, left_gripper_trigger_in, left_gripper_squeeze_in, right_gripper_trigger_in, right_gripper_squeeze_in,
        ...
        for idx, id in enumerate(Brainco_Left_Hand_JointIndex):
            ...
            self.left_hand_msg.cmds[id].kp = self.kp
            self.left_hand_msg.cmds[id].kd = self.kd
        for idx, id in enumerate(Brainco_Right_Hand_JointIndex):
            self.right_hand_msg.cmds[id].q = 0.0
            self.right_hand_msg.cmds[id].dq = 1.0
            self.right_hand_msg.cmds[id].kp = self.kp
            self.right_hand_msg.cmds[id].kd = self.kd
